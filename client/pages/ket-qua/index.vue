		...
		methods:{
			async add_domain(){
				if(!this.domain_add_value.includes('@')) return this.$message({message:'Phải nhập đúng định dạng @name.com',type:'error'})
					let {data} = await this.$axios.post('http://localhost:8080/api/domain',{name:this.domain_add_value});
			...
			async handleDelete(index,row){
				await this.$axios.delete('http://localhost:8080/api/domain/'+row.id);
				this.domain = this.domain.filter(({id})=>id !== row.id);

				return this.$message({message:'Đã xóa '+row.name,type:'success'})
			},
			async loadDomain(){
				let {data} = await this.$axios.get('http://localhost:8080/api/domain');
				this.domain = data;
			},
			...
			loadCount:async function(command = ''){
				let {data} = await this.$axios.get('http://localhost:8080/api/result/count?command='+command);
				return this.count = data;
			},
			loadData:async function(pagination = 1){

				let {data} = await this.$axios.get('http://localhost:8080/api/result?limit=30&where={"email":{"contains":"'+this.command+'"}}&skip='+(30*(pagination-1)));
			...
			download(hold){
				this.$axios({
					method: 'post',
					url: 'http://localhost:8080/api/email/download',
